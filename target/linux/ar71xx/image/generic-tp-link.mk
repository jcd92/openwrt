...
define Device/archer-d7-v1
  ...
  TPLINK_HVERSION := 3
  MTDPARTS := spi0.0:128k(u-boot)ro,1552k(kernel),14384k(rootfs),64k(radioDECT)ro,64k(config)ro,64k(romfs)ro,64k(rom)ro,64k(art)ro
  KERNEL := kernel-bin | patch-cmdline | lzma
  KERNEL_INITRAMFS := kernel-bin | patch-cmdline | lzma | \
	tplink-v2-header -s -V "ver. 1.0"
  IMAGES := sysupgrade.bin
  IMAGE/sysupgrade.bin := tplink-archer-d7-image -s -V "ver. 1.0" | \
